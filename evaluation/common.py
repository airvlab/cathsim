...
def get_tb_logs(path: Path, n_interpolations: int = 30,
                max_timesteps: int = 600_000, length_threshold: int = 150):
    ...
    for log in log_paths:
        ...
        tensorboard_log.reindex(
            np.arange(
                0, max_timesteps, max_timesteps / n_interpolations
            ), method='nearest'
        )
    ...
    for phantom in phantoms:
        ...
        for target in targets:
            algorithms = [algorithm for algorithm in target.iterdir() if
                          ((algorithm_name is None or algorithm.name == algorithm_name)) and algorithm.is_dir()]
