...
class Trajectory:
    def __init__(self, keys=None, image_size=480, image=None):
        ...
        self.image = np.zeros((self.image_size, self.image_size, 3)) if image is None else image
    ...
    def plot_head_positions(self, ax, key='head_positions', add_line: bool = False,
                            **kwargs) -> plt.Axes:
        data = self.data[key]
        camera_matrix = {
            480: np.array([
            ...
            80: np.array([
                [-96.56854249, 0., 39.5, -8.82205627],
                [0., 96.56854249, 39.5, -17.99606781],
                [0., 0., 1., - 0.15],
            ])
        }

        data = np.apply_along_axis(point2pixel, 1, data, camera_matrix=camera_matrix[80])
...
def process_human_data(paths=List[Path], mapping: Callable = lambda x: x) -> Trajectories:
