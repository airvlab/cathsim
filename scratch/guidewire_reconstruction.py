...
def load_data(path: Path):
    return zip(
        sorted(list(path.glob("*_top.npy")), key=lambda x: x.name),
        sorted(list(path.glob("*_side.npy")), key=lambda x: x.name),
        sorted(list(path.glob("*_geom_pos.npy")), key=lambda x: x.name),
    )
...
def triangulate_points(P1, P2, points1, points2):
    ...
    points_3d = cv2.triangulatePoints(
        P1, P2, points1_homogeneous.T, points2_homogeneous.T
    )
...
if __name__ == "__main__":
    ...
    for d in data:
        ...
        segment_height_pixel_top = real_to_pixel_length_from_matrix(
            P_top, distance_to_object_m_top, segment_height_m
        )
        segment_height_pixel_side = real_to_pixel_length_from_matrix(
            P_side, distance_to_object_m_side, segment_height_m
        )
