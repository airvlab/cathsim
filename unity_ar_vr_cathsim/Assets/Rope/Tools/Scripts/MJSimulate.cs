...
public class MJSimulate : MonoBehaviour 
{

    // prevent repeated instances
    private static MJSimulate instance;
    private MJSimulate() {}
    public static MJSimulate Instance
    {
        get 
        {
            if( instance==null )
            {
                instance = new MJSimulate();
                return instance;
            }
            else
                throw new System.Exception("MJSimulate can only be instantiated once");
        }
    }
