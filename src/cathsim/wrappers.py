...
register(
    ...
    entry_point="cathsim.wrappers:CathSim",
...
class DMEnvToGymWrapper(gymnasium.Env):
    ...
    def _render_frame(self, mode="rgb_array", image_size=None):
    ...
    def render(self, mode: str = "rgb_array", image_size: int = None) -> np.ndarray:
        ...
        if mode == "rgb_array":
            return self._render_frame(mode, image_size)
        else:
...
class CathSim(gymnasium.Env):
    spec = EnvSpec(
        "CathSim-v0", entry_point="cathsim.wrappers:CathSim", max_episode_steps=300
    )
    ...
    def __init__(
        ...
        self._env = make_dm_env("phantom3", **task_kwargs)
        self.metadata = {
            "render.modes": ["rgb_array"],
            "video.frames_per_second": round(1.0 / self._env.control_timestep()),
        }
    ...
    def render_frame_2(self, mode="rgb_array", image_size=None):
        image_size = image_size or self.image_size
        img = self._env.physics.render(height=image_size, width=image_size, camera_id=0)
        return img
...
def main():
    ...
    for i in range(10):
        ...
        while not done:
            ...
            print(reward)
            image = env.render_frame_2(image_size=480)
